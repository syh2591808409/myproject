...
import com.example.myproject.domain.ResObject;
...
import com.example.myproject.service.impl.PageUtil;
...
public class NewsController {
    ...
    private Admin adminRes;

    @RequestMapping("/admin/newsManage_{pageCurrent}_{pageSize}_{pageCount}")
    public String newsManage(News news,@PathVariable Integer pageCurrent, @PathVariable Integer pageSize, @PathVariable Integer pageCount, Model model,@RequestParam("userName") String userName) {
        Admin admin = adminService.findByNameSingle(userName);
        model.addAttribute("userName",admin.getUserName());
        model.addAttribute("headPicture",admin.getHeadPicture());
        //判断
        ...
        String pageHTML = PageUtil.getPageContent("newsManage_{pageCurrent}_{pageSize}_{pageCount}?userName="+userName+"&title="+news.getTitle()+"&category="+news.getCategory()+"&commendState="+news.getCommendState()+"&orderBy="+news.getOrderBy(), pageCurrent, pageSize, pageCount);
    ...
    public String newsEditGet(Model model,News news,@RequestParam("userName")String userName) {
        adminRes=adminService.findByNameSingle(userName);
        Admin admin = adminService.findByNameSingle(userName);
        model.addAttribute("userName",admin.getUserName());
        model.addAttribute("headPicture",admin.getHeadPicture());
        NewsCategory newsCategory = new NewsCategory();
    ...
    public String newsEditPost(Model model, News news, @RequestParam MultipartFile[] imageFile, HttpSession httpSession) {
        ...
        System.out.println(adminRes.getUserName());
        return "redirect:newsManage_0_0_0"+"?userName="+adminRes.getUserName();
    }
